...
ColumnFamily.prototype.insert = function(key, columns, options, callback){
  this.createBatchInserter(options, callback).insert(key, columns).execute();
};

/**
 * Create an inserter that allows multiple inserts before execution
 *
 * @param {Object} options The options for the insert
 * @param {Function} callback The callback to call once complete
 */
ColumnFamily.prototype.createBatchInserter = function(options, callback){
  if (typeof options === 'function'){
    callback = options;
    options = {};
  }

  var self = this, batch = {}, estimatedBatchSize = 0,
      consistency = options.consistency || options.consistencyLevel || DEFAULT_WRITE_CONSISTENCY;

  return {
    estimatedBatchSize: function(){
      return estimatedBatchSize;
    },
    insert: function(key, columns){
      estimatedBatchSize += roughSizeOfObject(key) + roughSizeOfObject(columns);
      ...
      for(; i < len; i += 1){
        col = columns[i];
        valueMarshaller = self.getColumnValidator(col.name) || self.valueMarshaller;
      ...
      var marshalledKey = self.keyMarshaller.serialize(key).toString('binary');

      batch[marshalledKey] = {};
      batch[marshalledKey][self.definition.name] = mutations;

      return this;
    },
    execute: function(){
      self.connection.execute('batch_mutate', batch, consistency, callback);
